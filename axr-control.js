...
// 1.4: witness_suspension (ideiglenes, auto-lejaro witness-kizaras).
const CONTROL_RECORD_TYPES = ['key_succession', 'key_revocation', 'witness_set', 'witness_revocation', 'witness_suspension'];
...
function verifyControlRecord(rec, rootAnchor, expectedLogId) {
  ...
  const v = rec.record_type === 'key_revocation' ? succ.verifyKeyRevocation(rec, anchor)
    ...
    : rec.record_type === 'witness_suspension' ? succ.verifyWitnessSuspension(rec, anchor)
