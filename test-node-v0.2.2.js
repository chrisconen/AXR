#!/usr/bin/env node
// Teszt-harness az axr-n8n-node-hu-v0.2.2.js-hez.
...
const NODE_CODE = fs.readFileSync(path.join(__dirname, 'axr-n8n-node-hu-v0.2.2.js'), 'utf8');
...
function runNode(env) {
  const fn = new Function('$', '$input', 'require', NODE_CODE);
  return fn(env.$, env.$input, require);
}
...
process.env.AXR_DIR = dir2; // nincs benne signing-key.pem
...
process.env.AXR_DIR = dir; // ervenyes kulcs vissza
